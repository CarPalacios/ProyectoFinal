...
@Slf4j
@Service
public class WithdrawalServiceImpl extends CrudServiceImpl<Withdrawal, String> implements InterfaceWithdrawalService {
	...
	public Mono<Withdrawal> createWithdrawal(Withdrawal withdrawal) {
		...
		Mono<Purchase> purchaseDatabase = purchaseService.findByCardNumber(withdrawal.getPurchase().getCardNumber())
				.switchIfEmpty(Mono.error(new RuntimeException("NUMERO DE TARJETA NO EXISTE")));

		Mono<Account> accountDatabase = accountService.findByAccountNumber(withdrawal.getAccount().getAccountNumber())
				.switchIfEmpty(Mono.error(new RuntimeException("NUMERO DE CUENTA NO EXISTE")));

		return purchaseDatabase.flatMap(purchase -> {
			...
			return accountDatabase.flatMap(account -> {
				...
				return service.create(withdrawal).map(createdObject -> createdObject)
	...
	public Mono<List<Withdrawal>> findAllFallback(Exception ex) {
		...
		list.add(Withdrawal.builder().id(ex.getMessage()).build());
	...
	public Mono<Withdrawal> createFallback(Withdrawal withdrawal, Exception ex) {
		...
		log.info("Withdrawal with account number {} could not be created, " + "retornando fallback",
				withdrawal.getAccount().getAccountNumber());

		return Mono.just(Withdrawal.builder().id(ex.getMessage())
				.amount(Double.parseDouble(withdrawal.getPurchase().getCardNumber()))
				.description(withdrawal.getAccount().getAccountNumber()).build());
