...
public class TransactionConsumer {
	...
	public Disposable retrieveCreatedAccount(String data) throws JsonProcessingException {
		...
		return Mono.just(account).log().flatMap(accountService::update).subscribe();
	...
	public Disposable retrieveCreatedWithdrawal(String data) throws JsonProcessingException {
		...
		return Mono.just(withdrawal).map(w -> {
			...
			return transactionService.create(Transaction.builder().account(w.getAccount()).purchase(w.getPurchase())
					.description(w.getDescription()).transactionType("RETIRO").transactionAmount(w.getAmount())
					.commission(commissionFin).transactionDate(LocalDateTime.now()).build()).block();
	...
	public Disposable retrieveCreatedDeposit(String data) throws JsonProcessingException {
		...
		return Mono.just(deposit).map(d -> {
			...
			return transactionService.create(Transaction.builder().account(d.getAccount()).purchase(d.getPurchase())
					.description(d.getDescription()).transactionType("DEPOSITO").transactionAmount(d.getAmount())
					.commission(commissionFin).transactionDate(LocalDateTime.now()).build()).block();
	...
	public Disposable retrieveCreatedCreditConsumer(String data) throws JsonProcessingException {
		...
		return Mono.just(creditConsumer)
				.map(c -> transactionService.create(Transaction.builder().purchase(c.getPurchase())
						.description(c.getDescription()).transactionType("CONSUMO TARJETA CREDITO")
						.transactionAmount(c.getAmount()).transactionDate(c.getConsumDate()).build()).block())
				.flatMap(a -> {
	...
	public Disposable retrieveCreatedCreditPayment(String data) throws JsonProcessingException {
		...
		return Mono.just(creditPayment)
				.map(p -> transactionService.create(Transaction.builder().purchase(p.getPurchase())
						.description(p.getDescription()).transactionType("PAGO TARJETA CREDITO")
						.transactionAmount(p.getAmount()).transactionDate(p.getPaymentDate()).build()).block())
				.flatMap(a -> {
